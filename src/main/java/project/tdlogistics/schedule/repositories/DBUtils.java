...
public class DBUtils {
    ...
    public <T> T findOneUnion(String table, List<String> fields, List<Object> values, Class<T> type) {
        ...
        if (fields == null || values == null || fields.isEmpty() || values.isEmpty()) {
            ...
            try {
                ...
                List<T> results = (List<T>) namedParameterJdbcTemplate.query(query, new ScheduleRowMapper());
            ...
            try {
                @SuppressWarnings("unchecked")
                List<T> results = (List<T>) namedParameterJdbcTemplate.query(queryBuilder.toString(), params,
    ...
    @SuppressWarnings("unchecked")
    public <T> List<T> find(String table, List<String> fields, List<Object> values, boolean desc, Integer limit,
            Integer offset, Class<T> type) {
        String query;
        MapSqlParameterSource params = new MapSqlParameterSource();

        if (fields != null && values != null && !fields.isEmpty() && !values.isEmpty()) {
            String whereClause = String.join(" AND ",
                    fields.stream().map(field -> field + " = :" + field).toArray(String[]::new));
        ...
        return (List<T>) namedParameterJdbcTemplate.query(query, params, new ScheduleRowMapper());
    }
    ...
    public int update(String table, List<String> fields, List<Object> values, List<String> conditionFields,
        ...
        if (fields == null || values == null || conditionFields == null || conditionValues == null ||
            ...
            throw new IllegalArgumentException(
                    "Fields, values, conditionFields, and conditionValues must not be empty");
        }

        String setClause = String.join(", ", fields.stream().map(field -> field + " = ?").toArray(String[]::new));
        String whereClause = String.join(" AND ",
                conditionFields.stream().map(field -> field + " = ?").toArray(String[]::new));
    ...
    public int updateOne(String table, List<String> fields, List<Object> values, List<String> conditionFields,
            List<Object> conditionValues) {
        if (fields == null || values == null || conditionFields == null || conditionValues == null ||
                fields.isEmpty() || values.isEmpty() || conditionFields.isEmpty() || conditionValues.isEmpty()) {
            throw new IllegalArgumentException(
